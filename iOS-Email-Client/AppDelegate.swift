...
extension AppDelegate: MessagingDelegate {
    ...
    func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable : Any], fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) {
        ...
        inboxVC.getPendingEvents(nil) { (success) in
            self.createEmailNotification(emailKey: key, account: inboxVC.myAccount)
            completionHandler(.newData)
        }
    }
    
    func createEmailNotification(emailKey: Int, account: Account) {
        SharedDB.refresh()
        guard let email = DBManager.getMail(key: emailKey, account: account) else {
            return
        }
